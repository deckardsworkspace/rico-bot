...
async def list_personal(self, ctx):
    ...
    if not len(ctx.message.mentions):
        await get_recommendations(ctx, self.db, ctx.author.name, ctx.author.avatar.url,
                                  server=False, entity_id=str(ctx.author.id))
    else:
        user = ctx.message.mentions[0]
        await get_recommendations(ctx, self.db, user.name, user.avatar.url, server=False, entity_id=str(user.id))
...
async def list_guild(self, ctx):
    ...
    await get_recommendations(ctx, self.db, ctx.guild.name, ctx.guild.icon.url, server=True)
