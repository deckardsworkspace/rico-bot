...
def check_spotify_url(url: str) -> bool:
    return re.match(r"(https?://open\.)*spotify(\.com)*[/:]+(track|artist|album|playlist)[/:]+[A-Za-z0-9]+", url)
...
def parse_spotify_url(url: str, valid_paths: list[str] = ["track", "album", "artist", "playlist"]) -> tuple[str, str]:
